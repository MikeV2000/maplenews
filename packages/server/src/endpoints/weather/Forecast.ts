...
export async function Forecast(req: any, data: Record<string, unknown>, db: Db): Promise<ForecastType | undefined> {
	const {units} = data;
	...
	return cache(db, 30, 'forecast', {lat, lon}, async () => {
		const response = await fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&appid=${environment.openWeatherMapApiKey}&units=${units}`);
